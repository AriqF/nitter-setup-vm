...
async def click_form_continue(tab, input_selector):
    """Click Continue in the same form as the active input."""
    selector = json.dumps(input_selector)
    clicked = await tab.evaluate(
        f"""(() => {{
            const input = document.querySelector({selector});
            const form = input && input.closest('form');
            const label = form && [...form.querySelectorAll('p')]
    ...
    if not clicked:
        control = await tab.find("Continue", best_match=True, timeout=20)
        if control is None:
            raise Exception("Could not find the active Continue control")
        await control.click()
...
async def login_and_get_cookies(username, password, totp_seed=None, headless=False):
    ...
    try:
        # X redirects the old /i/flow/login URL to its current web onboarding
        # flow.  CSS selectors are required here; tab.find() searches text.
        print(f"[*] Entering username {username}...", file=sys.stderr)
        username_input = await tab.select(
            'input[name="username_or_email"]', timeout=20
        )
        await username_input.send_keys(username)
        await click_form_continue(tab, 'input[name="username_or_email"]')

        print("[*] Entering password...", file=sys.stderr)
        password_input = await tab.select(
            'input[autocomplete="current-password"]', timeout=20
        )
        await password_input.send_keys(password)
        await click_form_continue(tab, 'input[autocomplete="current-password"]')

        # Handle 2FA if needed
        code_input = None
        authenticated = False
        for _ in range(60):
            code_input = await tab.query_selector(
                'input[autocomplete="one-time-code"]'
            )
            cookies = await browser.cookies.get_all()
            authenticated = any(cookie.name == "auth_token" for cookie in cookies)
            if code_input or authenticated:
                break
        ...
        if code_input:
            ...
            await click_form_continue(tab, 'input[autocomplete="one-time-code"]')
            await asyncio.sleep(3)
        elif not authenticated:
            page_text = " ".join(
                (await tab.evaluate("document.body.innerText")).split()
            )
            raise Exception(f"Login stopped at an unexpected X page: {page_text[:300]}")
